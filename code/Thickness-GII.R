...
for (z in 1:numTH){
  A<-rma.uni(yi=Y[,z],vi=Vi[,z],mods=cbind(as.numeric(GI)),method = "PM")
  Am[z,1]=A$beta[2]
  Am[z,2]=A$pval[2]
  rm(A)
}
...
for (z in 1:numTH){
  Ag<-rma.uni(yi=Y[,z],vi=Vi[,z],mods=cbind(as.numeric(GI),log(as.numeric(GDP))),method = "PM")
  Amg[z,1]=Ag$beta[2]
  Amg[z,2]=Ag$pval[2]
  rm(Ag)
}
...
A1<-rma.uni(yi=Y[,z],vi=Vi[,z],mods=as.numeric(GI),method = "PM")

Tright1<-as.data.frame(cbind(as.numeric(GI),Y[,z],as.numeric(DEM$N)))
...
bp<- ggplot(Tright1, aes(x=V1, y=V2, size = V3)) +
  ...
  annotate("text",x=1, y=-0.095, label= "paste(italic(I) ^ 2, \" = 21.87%\")", parse = TRUE,size=7,hjust=1) + 
  annotate("text",x=1, y=-0.11, label= "paste(italic(R) ^ 2, \" = 4.34%\")", parse = TRUE,size=7,hjust=1) + 
...
A2<-rma.uni(yi=Y[,z],vi=Vi[,z],mods=as.numeric(GI),method = "PM")

Tright2<-as.data.frame(cbind(as.numeric(GI),Y[,z],as.numeric(DEM$N)))
...
bp_r1<- ggplot(Tright2, aes(x=V1, y=V2, size = V3)) +
  ...
  annotate("text",x=0.3, y=-0.18, label= "paste(italic(P), \"-value (fdr) = 0.04\")", parse = TRUE,size=5) + 
  annotate("text",x=0.2, y=-0.21, label= "paste(italic(I) ^ 2, \" = 18.37%, \")", parse = TRUE,size=5,hjust=1) + 
  annotate("text",x=0.2, y=-0.21, label= "paste(italic(R) ^ 2, \" = 29.73%\")", parse = TRUE,size=5,hjust=0) + 
  ylim(c(-0.21,0.22))+
...
Tright3<-as.data.frame(cbind(as.numeric(GI),Y[,z],as.numeric(DEM$N)))
...
bp_r2<- ggplot(Tright3, aes(x=V1, y=V2, size = V3)) +
  ...
  annotate("text",x=0.3, y=-0.18, label= "paste(italic(P), \"-value (fdr) = 0.012\")", parse = TRUE,size=5) + 
  annotate("text",x=0.3, y=-0.21, label= "paste(italic(I) ^ 2, \" = 19.29%, \")", parse = TRUE,size=5,hjust=1) + 
  annotate("text",x=0.3, y=-0.21, label= "paste(italic(R) ^ 2, \" = 36.29%\")", parse = TRUE,size=5,hjust=0) + 
  ylim(c(-0.21,0.22))+
...
Tright4<-as.data.frame(cbind(as.numeric(GI),Y[,z],as.numeric(DEM$N)))
...
bp_r3<- ggplot(Tright4, aes(x=V1, y=V2, size = V3)) +
  ...
  geom_point(alpha=0.3) + scale_size_area(max_size=10, name="Number of participants",breaks=c(100,200,300,400)) +
  xlab(" ") + ylab("Difference in cortical thickness") + theme_light() + 
  annotate("text",x=0.3, y=-0.18, label= "paste(italic(P), \"-value (fdr) = 0.0495\")", parse = TRUE,size=5) + 
  annotate("text",x=0.3, y=-0.21, label= "paste(italic(I) ^ 2, \" = 15.96%, \")", parse = TRUE,size=5,hjust=1) + 
  annotate("text",x=0.3, y=-0.21, label= "paste(italic(R) ^ 2, \" = 30.25%\")", parse = TRUE,size=5,hjust=0) + 
  ylim(c(-0.21,0.22))+
